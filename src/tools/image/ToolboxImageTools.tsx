    ...
    <div className="space-y-4">
      ...
      {svgText.trim() && (
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="text-[11px] font-mono  text-muted-foreground">Output: {Math.round(svgDims(svgText).w * scale)} × {Math.round(svgDims(svgText).h * scale)} px</span>
        </div>
      )}
      ...
      {!img ? (
        ...
        <>
          <div className="rounded-2xl border   bg-card border-border p-4 flex items-center gap-4">
            <img src={img.src} alt="Source" className="w-16 h-16 object-contain rounded-lg  bg-muted border  border-border" />
            <div className="text-xs  text-muted-foreground space-y-0.5">
              <p className="font-mono  text-muted-foreground">{img.naturalWidth} × {img.naturalHeight}</p>
        ...
        <>
          <div className="rounded-xl border   bg-card border-border p-4 space-y-3">
            <div className="flex flex-wrap items-center gap-3 text-xs  text-muted-foreground">
              ...
              <span className="font-mono  text-muted-foreground">{base64Only.length.toLocaleString()} chars</span>
              <label className="flex items-center gap-1.5 cursor-pointer select-none">
                <input type="checkbox" checked={withPrefix} onChange={(e) => setWithPrefix(e.target.checked)} className="rounded text-primary" />
...
export const Base64ToImageTool: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border   bg-card border-border p-3 space-y-2">
        <label className="block text-xs  text-muted-foreground font-medium">Base64 string</label>
        <textarea
          value={input}
        ...
        <div className="flex items-center justify-between">
          <span className="text-[11px] font-mono  text-muted-foreground">{input.trim().length.toLocaleString()} chars</span>
        ...
        <>
          <div className="grid grid-cols-2 gap-3 text-center">
            <div className="rounded-xl border   bg-card border-border p-4">
              ...
              <p className="font-mono text-[13px]  text-muted-foreground">{formatBytes(file?.size ?? 0)}</p>
              <p className="font-mono text-[10px]  text-muted-foreground">{img.naturalWidth} × {img.naturalHeight} · {meta?.type.split('/')[1] ?? ''}</p>
            </div>
            <div className="rounded-xl border   bg-card border-border p-4">
              <p className="text-[10px] uppercase tracking-wider  text-muted-foreground font-semibold mb-1">After strip</p>
              <p className="font-mono text-[13px]  text-muted-foreground">{result ? formatBytes(result.blob.size) : '-'}</p>
              <p className="font-mono text-[10px]  text-muted-foreground">{result ? `${Math.max(0, Math.round((1 - result.blob.size / (file?.size ?? 1)) * 100))}% smaller` : 'waiting'}</p>
            </div>
          </div>
          <div className="rounded-xl border   bg-card border-border p-4 space-y-2">
      ...
      {!img ? (
        ...
        <>
          <div className="rounded-2xl border   bg-card border-border overflow-hidden">
            <img src={img.src} alt="Palette source" className="max-h-[240px] w-full object-contain  bg-muted" />
          </div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {palette.map((hex, i) => (
                <div key={i} className="rounded-xl overflow-hidden border  border-border">
                  ...
                  <button onClick={() => navigator.clipboard.writeText(hex)} className="w-full py-1.5 text-[10px] font-mono  text-muted-foreground hover: hover:bg-muted">{hex}</button>
                </div>
              ))}
            </div>
          </div>
