// Article bodies for the "pdf" topic, split out of posts/pdf.ts.
//
...
export const PDF_BODIES: Record<string, string> = {
  'how-pdf-compression-actually-works': `You drag a 42 MB PDF into a compressor, wait, and get back a 41.6 MB file. The same tool takes someone else's 30 MB scan down to 2 MB. Neither result is a bug, the two files are heavy for completely different reasons.
...
The practical benefit is speed as much as privacy: a 50 MB file never has to travel up and back down a connection before you see the result.`,
  'merge-pdf-files-without-uploading': `Merging two PDFs sounds like concatenating two files. It is not. A PDF is an object graph with an index at the end, so merging means copying objects out of one graph into another, renumbering every reference, and rebuilding the index. Most of the surprises come from what happens to the objects that are not simple page content.
...
If you only need a handful of pages from each source, [extract those pages](/tool/pdf-page-extractor) first. Merging less is always faster than compressing more.`,
  'how-to-redact-a-pdf-properly': `There is a specific, repeated, very public failure mode in document handling: someone opens a PDF, draws filled black rectangles over the sensitive passages, saves, and publishes. The passages are then recovered by anyone who selects the page and pastes it into a text editor.
...
NextTool's [PDF redaction tool](/tool/pdf-redact) removes the underlying content in your browser and never transmits the file; the [flatten](/tool/pdf-flatten) and [metadata](/tool/pdf-metadata) tools cover the other two steps. Whatever tool you use, run the copy-paste test on the result before it leaves your machine.`,
  'ocr-scanned-pdf-explained': `There are two completely different kinds of PDF, and telling them apart takes two seconds: try to select a sentence with your cursor. If the text highlights, the document contains real text. If nothing happens, you are looking at pictures of pages and no amount of "extract text" will help, you need OCR.
...
That is how NextTool's [PDF OCR](/tool/pdf-ocr) and [image OCR](/tool/ocr-image) work. It is slower than a datacentre GPU, and the trade is that your medical records, contracts and ID documents, which is overwhelmingly what people OCR, never leave the machine. Expect a few seconds per page, and expect the first run to pause while the language data downloads.`,
  'reduce-pdf-file-size-for-upload-limits': `Upload limits are arbitrary and unforgiving. Visa application portals commonly cap documents at 1–2 MB. University admission systems often say 500 KB. Gmail refuses attachments over 25 MB. Meanwhile a single phone-scanned page can easily be 4 MB on its own.
...
[Compression](/tool/pdf-compress), [page extraction](/tool/pdf-page-extractor) and [PDF-to-image conversion](/tool/pdf-to-jpg) on NextTool all run inside your browser, so the document only ever reaches the destination you actually intended.`,
  'taking-pages-out-of-a-pdf': `You need pages 12 to 18 of a 200-page report. There are three buttons in front of you and they all appear to do it. They do not produce the same thing, and the difference matters more once you have the output.
...
All of these are page-content and dictionary edits, which is why [numbering](/tool/pdf-page-numbers), [headers and footers](/tool/pdf-header-footer), [cropping](/tool/pdf-crop) and [rotation](/tool/pdf-rotate) all run locally here. A submission bundle is usually the last document you would want to hand to an anonymous web service, and none of this needs one.`,
  'making-a-pdf-from-something-else': `Three different jobs get called "make a PDF": turn this web page into one, turn this text file into one, and give me back the form I just filled in. They share an output format and almost nothing else.
...
Everything above runs in the browser tab: rendering HTML, laying out text, setting field values and rebuilding a cross-reference table are all things a modern browser does without a server. Given that filled forms are, by definition, documents with your details in them, that is the property worth having.`,
  'images-to-pdf-and-comparing-versions': `You photograph twelve pages of a signed agreement with your phone. You now have twelve JPEGs, in an order determined by how fast you were working, each 4000 pixels wide and 3 MB. Turning that into something a solicitor will accept involves four decisions, and it is worth making them deliberately.
...
All of these run in the browser here. Given that the documents people photograph page by page are overwhelmingly contracts, identity documents and medical records, that is the property worth having.`,
  'preparing-files-for-print': `You send a poster to a printer. It comes back with a white strip down one edge, the photographs look soft, and the vivid blue in your logo has arrived as a flat purple-grey. Three separate problems, and all three are predictable before you send the file.
...
Those are separate problems, and they are the ones covered by [a backup that works](/blog/a-backup-that-actually-works): more than one copy, on more than one kind of media, with checksums so you can prove the file you restore is the file you filed.`,
  'getting-tables-out-of-a-pdf': `You select a table in a PDF, copy it, paste into a spreadsheet, and get every value stacked in column A in an order that does not match what you saw.
...
[Text extraction](/tool/pdf-extract-text), [OCR](/tool/pdf-ocr) and the [CSV tools](/tool/csv-editor) here all run in the browser, which is the relevant property given that the tables people most often need out of a PDF are financial statements and payroll reports.`,
  'password-protecting-a-pdf': `"Password protect this PDF" describes two different operations in the PDF specification, and the gap between them explains why a protected file sometimes opens without any prompt at all.
