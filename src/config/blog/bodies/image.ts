// Article bodies for the "image" topic, split out of posts/image.ts.
//
...
export const IMAGE_BODIES: Record<string, string> = {
  'jpeg-png-webp-avif-which-format': `Image formats are not interchangeable containers for the same thing. Each one made a different bet about what kind of picture it would hold, and using the wrong one costs you either file size or visual quality, sometimes both.
...
One rule regardless of tool: **always convert from the highest-quality original you have.** Converting a JPEG to WebP inherits every artefact the JPEG already had, then adds its own.`,
  'how-image-compression-works': `Compression is the art of storing the same picture in fewer bytes. There are two fundamentally different ways of doing it, and mixing them up is where most confusion begins.
...
For a batch, [bulk resizing](/tool/batch-resize) before compressing usually gets you further than any quality setting alone.`,
  'resize-vs-compress-images': `Two operations, constantly confused, with completely different effects on a file.
...
If you are preparing a gallery or a product catalogue, doing this by hand for sixty images is how it stops getting done. [Batch resizing](/tool/batch-resize) applies a single width to a whole folder in one pass, in the browser, without uploading any of them, which matters when the folder is client work or unpublished product photography.`,
  'remove-image-background-in-browser': `Cutting a subject out of a photograph used to be twenty minutes of careful path work. A segmentation model now does a credible job in a few seconds. It is worth knowing what it is actually doing, because that explains its failure modes precisely.
...
Expect a few seconds per image after the model loads, and expect a modern browser to be required, WebGPU support makes a substantial difference to speed.`,
  'optimise-images-for-web-performance': `On a typical content page, images account for the majority of transferred bytes. They are also the element most likely to be the Largest Contentful Paint, the metric that decides whether a page feels fast. Four decisions cover almost all of the available improvement.
...
Steps 2 to 4 are batch work. [Bulk resizing](/tool/batch-resize), [format conversion](/tool/image-convert) and [compression](/tool/image-compressor) all run locally in the browser, which for unreleased design work or client photography is the difference between a routine task and a data-handling decision.`,
  'favicon-sizes-and-formats': `Search for favicon sizes and you will find checklists of fourteen files, most of which exist for devices discontinued a decade ago. The real list is much shorter, and one entry on it is a vector.
...
Step four is the only one that is not mechanical, and it is the one that matters. All the mechanical parts run in the browser here, which for an unreleased brand is the difference between a build step and uploading your logo to a stranger's server before launch.`,
  'putting-things-on-top-of-images': `Adding a watermark, annotating a screenshot, making a meme and assembling a collage are the same operation underneath: draw something on top of an image and flatten the result to new pixels. What separates them is what happens if you get it wrong.
...
Which matters more than it sounds, because the images people annotate are disproportionately screenshots of things that should not be public, and the images people watermark are disproportionately unpublished work. Both are poor candidates for uploading to a free editor in exchange for a download link.`,
  'rotate-flip-and-sharpen': `A photo that looks perfectly upright in your gallery appears rotated 90 degrees when you put it on a web page. The file is not corrupt and nothing resized it. It has an orientation tag, and something along the way ignored it.
...
All of these are pixel operations on data your browser already has in memory, which is why none of them here require an upload. For a photograph of a document, a child, or an unreleased product, that is the whole argument.`,
  'data-uris-when-to-inline-an-image': `A data URI puts the image inside the document:
...
Both [encoding](/tool/image-to-base64) and [decoding](/tool/base64-to-image) are pure string operations on data the browser already holds, which is why neither needs an upload here. That is the useful property when the image in question is an unreleased logo or a screenshot pulled out of an internal API response.`,
  'cropping-well': `You upload a carefully composed photo. The site shows a square thumbnail with the top of someone's head missing. Nothing malfunctioned: the platform needed a square, your image was not one, and something had to choose what to throw away.
...
[Cropping](/tool/image-crop), [resizing](/tool/image-resize) and [batch resizing](/tool/batch-resize) here all run on the canvas in your own browser. For photographs of people, which is most of what gets cropped, not uploading them to an editing service in order to trim 200 pixels off the top is the sensible default.`,
  'passport-and-id-photo-specs': `You upload a photo to a visa portal and it comes back rejected, with a message that says the photo does not meet requirements and nothing else. The photo looks fine. It probably is fine as a photograph and wrong as a document.
...
Every step above is a pixel operation your browser can do. That is worth having here for an obvious reason: a passport photo is an identity document, and uploading one to a free resizing service in order to make it smaller hands your face, and your intent, to an unknown party before the official portal ever sees it.`,
  'social-preview-images': `You share a link in a group chat. The preview shows the middle third of your carefully composed image, the headline cut off at both ends, and a logo half out of frame. The same link on another platform looks fine.
...
Every step runs in this browser tab. That matters less for a public marketing image than for the ones built from unreleased product screenshots, which is what most preview cards actually contain in the week before a launch.`,
  'why-colours-look-different-on-each-screen': `You export a photo, it looks right, and the client says the reds are orange. You open it again and it still looks right. Both of you are looking at the same file.
...
The pipeline that works: edit wide, convert to sRGB and embed the profile for the web, convert to the printer profile for print, and check the result on a second device before anyone else sees it.`,
  'product-photos-for-listings': `A listing is rejected for "image does not meet requirements". The photo is sharp, well lit and looks professional. It is also 900 pixels wide on a platform that wants 1600, on a background that is very slightly grey.
...
Every step here runs in the browser, which for unreleased product photography is the difference between a routine export and publishing your catalogue to a third party before launch.`,
  'screenshots-for-bug-reports': `Someone sends a screenshot of an error. It is a full-desktop capture at 3840 pixels wide, the error is a small grey box in the corner, and the top of the frame shows an email notification containing a customer name.
...
All of it runs in this browser tab, which is the relevant property when the screenshot is of an internal admin panel with real customer records on it, which is what support screenshots very often are.`,
  'making-a-gif-worth-sending': `You export a five-second screen recording as a GIF and it comes out at 38 MB. The same clip as an MP4 is under a megabyte and looks better. This is not a bad exporter. It is the format doing exactly what it was designed to do in 1987.
