...
Every calculation here is arithmetic on numbers you typed, so none of it needs a server. That is worth having for the loan and tax cases in particular, where the figures being entered are salary, outstanding debt and turnover.`,
  'dates-numbers-across-locales': `A colleague sends a spreadsheet with a column of dates reading \`03/04/2026\`. In London that is the third of April. In New York it is the fourth of March. Nothing in the file distinguishes them, and both readers are confident.
...
Point six is the cheapest of them and the one that would have prevented every problem described above.`,
  'mean-median-and-percentiles': `Support tickets are answered in a mean time of four hours. Most are answered in twenty minutes; a handful took three days. The mean describes neither group and no individual ticket.
...
And the standing caution: a difference between two averages is not automatically a real difference. Small samples move around on their own, and two numbers that differ by a point may be describing the same underlying thing on two different days. Reporting the count is what lets a reader judge that for themselves.`,
  'compound-interest-and-saving': `Two accounts both advertise 12% a year. One pays it once at the end; the other pays 1% every month. After a year the second has paid 12.68%.
...
The [unit converter](/tool/unit-converter) and [scientific calculator](/tool/scientific-calc) here run locally, so checking a figure is instant and the numbers you are checking stay on your machine.`,
  'gst-invoices-and-rounding': `An invoice is arithmetic anyone can do and a surprising number of systems get wrong. Not because the percentages are hard, but because rounding, ordering and inclusive pricing interact.
